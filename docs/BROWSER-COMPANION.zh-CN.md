...
7. 选择已知的工作区、`profile` 与执行器。`auto` 会选择第一个健康且已安装/已配置的执行器。
...
`DEFAULT` 在扩展内部解析，使本地文件系统路径不会进入网页对话。派发模式有三档：`auto`（自动派发，默认）、`confirm`（暂存任务块，需回复「执行 / 是否派发」确认后派发）、`manual`（仅提示，点「派发」按钮执行）。自动派发启用时，伴侣会将信封发送给 AgentControlPlane，监控任务，并插入一个紧凑的最终结果块：
